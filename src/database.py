...
def init_db():
    ...
    try:
        if not db.query(User).filter_by(username='admin').first():
            admin = User(username='admin', password_hash=generate_password_hash(os.environ.get('ADMIN_PASSWORD', 'admin')))
            db.add(admin)
        rules = {'temp_max': (60.0, 'Absolute maximum allowed temperature setpoint (C)'), 'temp_min': (0.0, 'Absolute minimum allowed temperature setpoint (C)'), 'pressure_max': (8.0, 'Absolute maximum allowed pressure setpoint (bar)'), 'pressure_min': (0.0, 'Absolute minimum allowed pressure setpoint (bar)')}
