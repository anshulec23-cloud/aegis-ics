...
DEVICE_KEYS = {'ESP32_001': os.environ.get('DEVICE_KEY_ESP32_001', secrets.token_hex(16)), 'ESP32_002': os.environ.get('DEVICE_KEY_ESP32_002', secrets.token_hex(16))}
...
class LocalRFModel:
    ...
    def predict_anomaly(self, telemetry: dict) -> bool:
        features = [[float(telemetry.get('temperature', 0.0)), float(telemetry.get('pressure', 0.0)), float(telemetry.get('vibration', 0.0) or telemetry.get('pressure', 0.0) if str(telemetry.get('device_id')) == 'ESP32_002' else 0.0), float(telemetry.get('hall_effect', 0.0) or telemetry.get('humidity', 0.0) if str(telemetry.get('device_id')) == 'ESP32_002' else 0.0), float(telemetry.get('current', 0.0) or telemetry.get('humidity', 0.0) if str(telemetry.get('device_id')) == 'ESP32_001' else 0.0)]]
...
def process_telemetry(payload: dict) -> bool:
    ...
    try:
        log = TelemetryLog(timestamp=payload.get('timestamp', time.time()), device_id=device_id, temperature=payload.get('temperature', 0.0), pressure=payload.get('pressure', 0.0), humidity=payload.get('humidity', 0.0), vibration=payload.get('vibration', 0.0), hall_effect=payload.get('hall_effect', 0.0), current=payload.get('current', 0.0), rssi=payload.get('rssi', 0.0), is_anomaly=is_anomaly)
        db.add(log)
        db.commit()
        if is_anomaly and device_id != 'unknown':
            ...
            if state and (not state.is_isolated):
                ...
                audit = AuditLog(user_id=None, action='AUTO_ISOLATION', location='SYSTEM', details=f'System automatically isolated device {device_id} due to {reason}.')
                db.add(audit)
                db.commit()
...
def login():
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password_hash, password):
            ...
            audit = AuditLog(user_id=user.id, action='LOGIN', location=location_str, details=f'User {username} successfully authenticated.')
            db.add(audit)
            db.commit()
            db.close()
...
def logout():
    ...
    if user_id:
        db = SessionLocal()
        audit = AuditLog(user_id=user_id, action='LOGOUT', location=location, details=f'User {username} logged out.')
        db.add(audit)
        db.commit()
        db.close()
    session.clear()
...
def setpoint():
    ...
    if not allowed:
        audit = AuditLog(user_id=user_id, action='SECURITY_VIOLATION_BLOCKED', location=location, details=f'Blocked attempt to set {cmd_type} to {value}. Reason: {reason}')
        db.add(audit)
        db.commit()
        db.close()
        return (jsonify({'success': False, 'error': reason}), 403)
    command_payload = {'command': 'setpoint', 'target': cmd_type, 'value': value, 'timestamp': datetime.now(timezone.utc).isoformat(), 'signature': ''}
    try:
    ...
    audit = AuditLog(user_id=user_id, action='CHANGE_SETPOINT', location=location, details=f'Changed {cmd_type} to {value}.')
    db.add(audit)
    db.commit()
    db.close()
...
def connect_com_port():
    ...
    try:
        ...
        gateway_thread = threading.Thread(target=serial_gateway.start_gateway, kwargs={'port': port if not mock else None, 'mock': mock, 'url': url, 'hmac_key': hmac_key}, daemon=True, name='serial-gateway')
        gateway_thread.start()
        db = SessionLocal()
        audit = AuditLog(user_id=session.get('user_id'), action='CONNECT_COM_PORT', location=session.get('location'), details=f'Connected hardware gateway to port {port}.')
        db.add(audit)
        db.commit()
        db.close()
...
def disconnect_com_port():
    try:
        ...
        serial_gateway.stop_gateway()
        db = SessionLocal()
        audit = AuditLog(user_id=session.get('user_id'), action='DISCONNECT_COM_PORT', location=session.get('location'), details='Disconnected hardware gateway manually.')
        db.add(audit)
        db.commit()
        db.close()
...
def isolate_device_v2():
    ...
    if state:
        state.is_isolated = True
        audit = AuditLog(user_id=session['user_id'], action='MANUAL_ISOLATION', location=session['location'], details='Operator manually isolated the device from the control loop.')
        db.add(audit)
        db.commit()
    db.close()
...
def rejoin_device_v2():
    ...
    if state:
        ...
        audit = AuditLog(user_id=session['user_id'], action='MANUAL_REJOIN', location=session['location'], details='Operator manually rejoined the device to the control loop.')
        db.add(audit)
        db.commit()
    db.close()
...
def download_report():
    ...
    try:
        ...
        return send_file(BytesIO(pdf_data), mimetype='application/pdf', as_attachment=True, download_name=f'aegis_scada_report_{int(time.time())}.pdf')
    except Exception as e:
...
def simulate_attack():
    ...
    if attack_type == 'stuxnet':
        log = TelemetryLog(timestamp=time.time(), device_id='ESP32_001', temperature=30.0, pressure=7.5, humidity=50.0, is_anomaly=False)
        db.add(log)
        db.commit()
        audit = AuditLog(user_id=user_id, action='SECURITY_VIOLATION_BLOCKED', location=location, details='Blocked attempt to set set_temp to 55.0. Reason: Stuxnet Prevention Policy - Temperature setpoint (55.0) rejected while system pressure is high (7.5 bar).')
        db.add(audit)
        db.commit()
        ...
        log = TelemetryLog(timestamp=time.time(), device_id='ESP32_001', temperature=58.0, pressure=4.0, humidity=50.0, is_anomaly=True)
        db.add(log)
        state = db.query(DeviceState).filter_by(device_id='ESP32_001').first()
        if state:
            state.is_isolated = True
        audit = AuditLog(user_id=None, action='AUTO_ISOLATION', location='SYSTEM', details='System automatically isolated device ESP32_001 due to invalid HMAC signature (Telemetry Spoofing / Injection Attack detected).')
        db.add(audit)
        db.commit()
        ...
        audit = AuditLog(user_id=user_id, action='SECURITY_VIOLATION_BLOCKED', location=location, details='Blocked unauthorized modification of safety thresholds: Attempted to set temp_max to 100.0 without Master Engineering credentials.')
        db.add(audit)
        db.commit()
...
def get_data():
    ...
    telemetry_data = [{'timestamp': t.timestamp, 'device_id': t.device_id, 'temperature': t.temperature, 'pressure': t.pressure, 'humidity': t.humidity, 'vibration': t.vibration or 0.0, 'hall_effect': t.hall_effect or 0.0, 'current': t.current or 0.0, 'rssi': t.rssi, 'is_anomaly': t.is_anomaly} for t in reversed(telemetry)]
    audit_data = [{'timestamp': a.timestamp.isoformat(), 'username': a.user.username if a.user else 'Unknown', 'action': a.action, 'location': a.location, 'details': a.details} for a in audit_logs]
    financials = calculate_financial_analytics(db)
    db.close()
    return jsonify({'telemetry': telemetry_data, 'audit_logs': audit_data, 'financials': financials})
