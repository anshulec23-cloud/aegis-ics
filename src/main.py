...
# ---------------------------------------------------------------------------
# Set desktop mode flag BEFORE importing app modules
# ---------------------------------------------------------------------------
...
def _configure_paths():
    ...
    if hasattr(sys, "_MEIPASS"):
        ...
        # Running from source — set cwd to version-two/
...
def _create_flask_server(port: int):
    ...
    """
    # Import the Flask app (triggers database init, etc.)
    from app import app
...
def _on_window_closing():
    ...
    if _webview_window is not None:
        try:
            ...
            pass
    return False  # Prevent the actual window destruction
...
# ---------------------------------------------------------------------------
# Global references (needed for cross-thread communication)
# ---------------------------------------------------------------------------
...
def main():
    ...
    try:
        ...
        pass  # Non-critical on non-Windows or during development
    ...
    # --- Step 5: Create the embedded browser window ---
    ...
    # Wire up close → hide to tray
    _webview_window.events.closing += _on_window_closing

    # --- Step 6: Start system tray (daemon thread) ---
    try:
    ...
    # --- Step 7: Start background update check ---
    ...
    # --- Step 8: Start pywebview event loop (BLOCKS on main thread) ---
    ...
    # Safely signal the serial gateway to release the COM port
    try:
    ...
    import time
    time.sleep(0.5) # Allow hardware UART lines to close cleanly
    
    # Force OS-level exit to immediately kill any hanging non-daemon Flask or background worker threads
    os._exit(0)
