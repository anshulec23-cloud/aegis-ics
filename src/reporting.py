...
def generate_incident_report_pdf(db_session, username, location):
    ...
    title_style = ParagraphStyle('TitleStyle', parent=styles['Heading1'], fontSize=20, leading=24, textColor=colors.HexColor('#000000'), spaceAfter=5)
    meta_style = ParagraphStyle('MetaStyle', parent=styles['Normal'], fontSize=9, leading=13, textColor=colors.HexColor('#86868b'), spaceAfter=15)
    h2_style = ParagraphStyle('H2Style', parent=styles['Heading2'], fontSize=12, leading=16, textColor=colors.HexColor('#1d1d1f'), spaceBefore=12, spaceAfter=6)
    body_style = ParagraphStyle('BodyStyle', parent=styles['Normal'], fontSize=8.5, leading=12, textColor=colors.HexColor('#1d1d1f'), spaceAfter=8)
    table_text = ParagraphStyle('TableText', parent=styles['Normal'], fontSize=8, leading=11, textColor=colors.HexColor('#1d1d1f'))
    table_header = ParagraphStyle('TableHeader', parent=styles['Normal'], fontSize=8, leading=11, textColor=colors.HexColor('#1d1d1f'), fontName='Helvetica-Bold')
    ...
    fin_data = [[Paragraph('Audit Category', table_header), Paragraph('Financial Impact', table_header), Paragraph('Security / Cost Breakdown', table_header)], [Paragraph('Incurred Incident Cost', table_text), Paragraph(f"${financials['incurred_cost']:,.2f}", table_text), Paragraph('Triage and investigation cost ($5,000 per violation attempt)', table_text)], [Paragraph('Projected Downtime Cost', table_text), Paragraph(f"${financials['expected_loss']:,.2f}", table_text), Paragraph('Liability projection based on dynamic Threat Index', table_text)], [Paragraph('Total Prevented Losses (Savings)', table_text), Paragraph(f"${financials['prevented_cost']:,.2f}", table_text), Paragraph('Savings from blocked centrifugal casing ruptures ($400,000 each)', table_text)]]
    t_fin = Table(fin_data, colWidths=[150, 100, 280])
    t_fin.setStyle(TableStyle([('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#f5f5f7')), ('ALIGN', (0, 0), (-1, -1), 'LEFT'), ('BOTTOMPADDING', (0, 0), (-1, -1), 5), ('TOPPADDING', (0, 0), (-1, -1), 5), ('GRID', (0, 0), (-1, -1), 0.5, colors.HexColor('#d2d2d7'))]))
    story.append(t_fin)
    ...
    audit_headers = [Paragraph('Timestamp', table_header), Paragraph('User', table_header), Paragraph('Action', table_header), Paragraph('Location Coords', table_header), Paragraph('Event Details', table_header)]
    ...
    for a in reversed(audit_logs):
        ...
        audit_rows.append([Paragraph(a.timestamp.strftime('%Y-%m-%d %H:%M:%S'), table_text), Paragraph(u_name, table_text), Paragraph(action_text, act_style), Paragraph(a.location, table_text), Paragraph(a.details or '', table_text)])
    t_audit = Table(audit_rows, colWidths=[90, 60, 110, 90, 180])
    t_audit.setStyle(TableStyle([('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#f5f5f7')), ('ALIGN', (0, 0), (-1, -1), 'LEFT'), ('BOTTOMPADDING', (0, 0), (-1, -1), 4), ('TOPPADDING', (0, 0), (-1, -1), 4), ('GRID', (0, 0), (-1, -1), 0.5, colors.HexColor('#e5e5ea'))]))
    story.append(t_audit)
    story.append(Spacer(1, 15))
    story.append(Paragraph('4. Recommended Mitigation Steps', h2_style))
    story.append(Paragraph('• <b>HMAC Credential Rotation</b>: Rotate secret device validation keys (DEVICE_KEY) on all ESP32 PLCs to prevent replay and injection vectors.<br/>• <b>Session Audit</b>: Inspect operator station coordinates to isolate coordinates outside permitted operational zones.<br/>• <b>Device Loop Check</b>: If a device status is MANUAL_ISOLATION, run hardware testing loop checks before rejoining the device to the operational loop.<br/>• <b>Stuxnet Mitigation</b>: Keep Aegis enforcer correlation rules active. The enforcer prevents centrifugal over-pressurization even under administrator credentials.', body_style))
    doc.build(story)
    buffer.seek(0)
