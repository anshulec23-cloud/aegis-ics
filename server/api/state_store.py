...
class StateStore:
    ...
    def isolate_device(self, device_id: str) -> dict[str, Any]:
        with self._lock:
            ...
            # Active isolation: update ACLs and reload broker
            try:
                from server.mqtt_broker.acl_manager import remove_device, reload_broker
                root = Path(__file__).resolve().parents[2]
                acl_path = root / "server" / "mqtt_broker" / "acl.conf"
                remove_device(acl_path, device_id)
                reload_broker()
            except Exception as e:
                print(f"[StateStore] WARNING: Failed to update broker ACLs: {e}")
    ...
    def rejoin_device(self, device_id: str) -> dict[str, Any]:
        with self._lock:
            ...
            # Active isolation restore: update ACLs and reload broker
            try:
                from server.mqtt_broker.acl_manager import add_device, reload_broker
                root = Path(__file__).resolve().parents[2]
                acl_path = root / "server" / "mqtt_broker" / "acl.conf"
                add_device(acl_path, device_id)
                reload_broker()
            except Exception as e:
                print(f"[StateStore] WARNING: Failed to update broker ACLs: {e}")
