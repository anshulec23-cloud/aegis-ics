...
class MicroSegmentationStore:
        ...
        with FileLock(str(self.path)) as _:
            if self.path.exists():
                try:
                    ...
                    # In case of corruption, fall back to empty set safely
    ...
    def isolate(self, device_id: str) -> None:
        self.load() # Reload to get the latest changes from other workers/processes
        self.isolated_devices.add(device_id)
        self.save()

    def rejoin(self, device_id: str) -> None:
        self.load() # Reload to get the latest changes
